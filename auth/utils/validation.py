...
def valid_register_data(auth_data):

    valid_result = {'birth': valid_user_age(auth_data.birth), 
                    'email': valid_user_email(auth_data.email), 
                    'phone': valid_user_phone(auth_data.phone)}
...
def check_login_user(login_data, db:Session):
...
def check_authentication(id, db:Session):
